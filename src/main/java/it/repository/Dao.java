...
public class Dao {
    ...
    public void createTableModel() {
        try {
            ...
            stmt.executeUpdate( "CREATE TABLE IF NOT EXISTS TRY (ID INTEGER, ID_COMB INTEGER ," +
                                    " ID_USER INTEGER, ID_GAME INTEGER, POS_ESATTA INTEGER, NUM_ESATTO INTEGER, VALORI_INSERITI VARCHAR(20)," +
                                    " Data_Operazione VARCHAR(30), PRIMARY KEY (ID, Data_operazione), FOREIGN KEY (ID_COMB) REFERENCES COMBINATION(ID))" +
                                    " FOREIGN KEY (ID_USER) REFERENCES UTENTE(ID), FOREIGN KEY (ID_GAME) REFERENCES COMBINATION(GAME_ID)");

            stmt.executeUpdate( "CREATE TABLE IF NOT EXISTS UTENTE (ID INTEGER PRIMARY KEY AUTO_INCREMENT, NOME VARCHAR (255))");

            stmt.executeUpdate( "CREATE TABLE IF NOT EXISTS GAME (ID INTEGER PRIMARY KEY AUTO_INCREMENT, ID_USER INTEGER, ID_COMBINATION INTEGER, RESULT INTEGER, " +
                                    "FOREIGN KEY (ID_COMBINATION) REFERENCES COMBINATION(ID), FOREIGN KEY (ID_USER) REFERENCES UTENTE(ID))");
